-- Migration to add personality_factors JSONB to the returned values of get_fallback_feed and get_discover_deck

-- Drop existing functions to allow signature changes (RETURNS TABLE changes)
DROP FUNCTION IF EXISTS public.get_fallback_feed(UUID, NUMERIC, INT);
DROP FUNCTION IF EXISTS public.get_discover_deck();

-- 1. Redefine get_fallback_feed with personality_factors JSONB column
...
-- 2. Redefine get_discover_deck to return serialized personality_factors
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'assistant_orientation', up.sexual_orientation, -- backwards compatibility alias
