-- Migration: Add profile details (vedic_sign, nakshatra, height_cm, looking_for) and all onboarding responses to discover feed
-- Drop functions with old signatures first
DROP FUNCTION IF EXISTS public.get_fallback_feed(uuid, numeric, int);

-- Redefine get_fallback_feed with new return table columns
...
-- Redefine get_discover_deck to return the new fields in jsonb_build_object
...
BEGIN
  ...
  -- exactly use up today's remaining swipes.
