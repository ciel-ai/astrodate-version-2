-- ============================================================================
-- Since the deck is now capped to remaining swipes (20260709130000), most
-- users exhaust their CARDS at the exact moment they exhaust their SWIPES --
-- via a successful final swipe, not a rejected one. The client's
-- `limitReached` state only ever gets set inside a REJECTED swipe's failure
-- branch, so this now-common case falls through to a generic "No more
-- profiles right now" that never mentions swipes, when the real reason is
-- "you're out for today."
--
-- Fix: expose meta.swipes_exhausted -- true when the number of cards
-- ACTUALLY served exactly equals today's remaining swipes, meaning
-- finishing this deck really does bottom out the quota. Deliberately NOT
-- based on the tier's configured deck_size: a thin candidate pool can serve
-- fewer cards than the swipe budget would allow, and in that case running
-- out of cards means "no more people right now", not "out of swipes" --
-- conflating the two would just move the same wrong-message bug elsewhere.
-- ============================================================================
