-- ============================================================================
-- Corrective follow-up to 20260720120000_fix_discover_deck_rewind_grace_period.sql.
--
-- That migration copied get_discover_deck()'s body from
-- 20260717150000_redefine_discover_deck.sql to apply the grace-period status
-- fix, but 20260717150000 was NOT the latest version -- two later same-day
-- migrations had already changed the function and were never reconciled with
-- each other:
--   - 20260717180000_add_personality_factors.sql added a
--     `personality_factors` field to the returned card JSON (read from
--     get_fallback_feed, which still has it -- unaffected by this).
--   - 20260717280000_fix_discover_deck_state_race.sql (applied ~10h later
--     the same day) fixed a real concurrency bug (two simultaneous calls
--     both seeing NOT FOUND on discover_deck_daily_state and racing on
--     INSERT) by switching to an atomic `INSERT ... ON CONFLICT DO UPDATE
--     ... RETURNING`, but was itself written from a pre-180000 copy of the
--     function and silently dropped `personality_factors` again.
-- 20260717280000's body (personality_factors missing, race-fixed) was what
-- was actually live before this session. 20260720120000 (this session, not
-- knowing about either later file) based its grace-period fix on
-- 20260717150000's older body, so the push re-introduced BOTH the missing
-- personality_factors field AND the read-then-insert race.
--
-- This migration is the union: 20260717280000's atomic upsert +
-- 20260717180000's personality_factors field + 20260720120000's grace-period
-- status broadening. No other logic changed.
-- ============================================================================
...
BEGIN
  ...
  ON CONFLICT (user_id, deck_date) DO UPDATE
    SET high_shown_ids = public.discover_deck_daily_state.high_shown_ids
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'personality_factors', CASE WHEN v_plan_slug = 'free' THEN NULL ELSE p.personality_factors END
      )
      ORDER BY ord.ord
