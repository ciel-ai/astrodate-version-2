-- ============================================================================
-- Two gaps in the Discover compatibility breakdown found while re-checking
-- against build plan Section 3's actual 3-way table:
--   Free    = total score only
--   Astro+  = full 45/45/10 + doshas
--   AstroX  = full + synastry "why you match"
--
-- The prior gating fix (20260709130000) only split free-vs-not -- Astro+ and
-- AstroX ended up identical, and doshas weren't surfaced to EITHER paid tier
-- despite get_match_score already computing them (in its `breakdown` JSONB,
-- via get_indian_compatibility). And "why you match" isn't a new feature to
-- build -- compute-synastry already writes a real narrative into
-- synastry_cache_details.compatibility_summary (buildSummary() in
-- compute-synastry/index.ts) and nothing ever reads it back out.
--
-- get_fallback_feed gets 4 new trailing columns (manglik_status, nadi_dosha,
-- bhakoot_dosha pulled from get_match_score's own breakdown -- no extra
-- get_indian_compatibility call needed; why_you_match from a LEFT JOIN onto
-- synastry_cache_details), same additive-column convention as every prior
...
-- ============================================================================
-- get_discover_deck() -- add the same 4 fields to each card, gated per
-- Section 3's actual 3-way split (not the 2-way free/paid split from
-- 20260709130000): doshas go to Astro+ AND AstroX; why_you_match is
-- AstroX-only, on top of everything Astro+ gets.
-- ============================================================================
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        -- Section 3: Free = total only. Astro+ and AstroX both get the full
        -- 45/45/10 split + doshas; AstroX additionally gets why_you_match.
