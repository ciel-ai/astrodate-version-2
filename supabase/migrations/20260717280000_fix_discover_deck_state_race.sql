-- ============================================================================
-- get_discover_deck() read-then-insert its per-user daily_state row with a
-- plain SELECT followed by a conditional INSERT (no ON CONFLICT). Two
-- concurrent calls for the same user on the same day (double-fetch from the
-- Discover screen mounting twice, a client retry racing the original
-- in-flight request, etc.) can both see NOT FOUND and both attempt the
-- INSERT, so the second one hits "duplicate key value violates unique
-- constraint discover_deck_daily_state_pkey" (PK is (user_id, deck_date) --
-- see 20260708170000_discover_deck_builder.sql). Fix: fold the
-- select-or-create into a single atomic upsert (ON CONFLICT DO UPDATE that
-- writes the row's own existing value back is a no-op that still lets
-- RETURNING hand back the current row either way), so there's no gap
-- between "check" and "act" for two concurrent transactions to land in.
-- ============================================================================
