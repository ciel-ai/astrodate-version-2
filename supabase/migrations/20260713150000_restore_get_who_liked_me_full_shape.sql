-- ============================================================================
-- Regression fix: 20260713120000_fix_membership_grants_and_grace_period.sql
-- replaced get_who_liked_me() with a much simpler tier-based rewrite (paid ->
-- full likers array, free -> just {tier:'basic', count}) that dropped fields
-- the client has always depended on (is_paid, likes, unseen_count,
-- free_reveal_used, free_reveal_available) and removed the per-row free
-- reveal / blur mechanic from 20260707040000 + 20260707060000 entirely.
--
-- Concretely this crashed the Likes tab for every free-tier user
-- (`data.likes.length` -- likes was undefined) and silently changed paid
-- behavior too (no more locked/blurred cards, no free-reveal affordance).
--
-- This restores the 20260707060000 implementation (the last known-good
-- version, confirmed against src/lib/likes.ts's WhoLikedMeResponse type and
-- src/context/likes.tsx's usage), carrying forward the ONE legitimate change
-- from 20260713120000: status is no longer required to be exactly 'active'
-- -- 'past_due'/'canceled' still count as paid until current_period_end, per
-- that migration's grace-period fix.
-- ============================================================================
